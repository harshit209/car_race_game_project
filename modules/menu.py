...
class MenuLicense(Menu):
  ...
  def __init__(self, titleFont, title, gap, itemFont):
    ...
    text = self.itemFont.render("Copyright (C) 2018 Harshit <ha6546ha@gmail.com>", 1, misc.lightColor)
    textRect = text.get_rect()
    textRect.centerx = misc.screen.get_rect().centerx
    textRect.y = y
    misc.screen.blit(text, textRect)
...
class MenuCredits(Menu):
  ...
  def __init__(self, titleFont, title, gap, itemFont):
    ...
    # Display license on different lines
    text = self.itemFont.render("Programming and tracks design: Harshit <ha6546ha@gmail.com>", 1, misc.lightColor)
    textRect = text.get_rect()
    textRect.centerx = misc.screen.get_rect().centerx
    textRect.y = y
    misc.screen.blit(text, textRect)
